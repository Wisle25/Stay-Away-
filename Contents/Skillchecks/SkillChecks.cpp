    ...
    DrawSkillCheck(false)
{
    ...
    ProgressBar = new Bar({
        .Size = { Cvt.P2PX(8.f), Cvt.P2PX(0.7f) },
        .Position = { Position.x - M_Texture.getSize().x * 2.f, Position.y - M_Texture.getSize().y * 1.12f },
        .Font = Font,
        .CharSize = Cvt.P2PChar(280)
    });
...
void SkillChecks::OnUpdate(const float& DeltaTime)
{
    ...
    ProgressBar->OnUpdate(Progress, MaxProgress, "Progress");
...
void SkillChecks::OnDraw(sf::RenderWindow* Window, unsh Status)
{
    // Inheritance
