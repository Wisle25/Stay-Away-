...
Tile::Tile(sf::Vector2f Position, float GridSize, const String& I_Texture, unsh I_Type):
...
const float Tile::GetBottom() const
{
    return STile.getPosition().y + static_cast<float>(Texture.getSize().y);
}

// \brief Getting some datas that needed to save map information
const String Tile::GetData() const
{
