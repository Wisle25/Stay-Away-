    ...
    Type(NORMAL)
{
    ...
    /* If you want to adjust the max world size, just adjust it here*/
    MaxWorldSize.x = 30;
    MaxWorldSize.y = 17;
...
void EditorState::MouseInputHandler(sf::Event* Event)
{
    ...
    if (Event->mouseButton.button == sf::Mouse::Left &&
        !Buttons["SAVE_BTN"]->GetGlobalBounds().contains(sf::Vector2f(MouseWindow)) &&
        !Buttons["LOAD_BTN"]->GetGlobalBounds().contains(sf::Vector2f(MouseWindow)) &&
        !Buttons["QUIT_BTN"]->GetGlobalBounds().contains(sf::Vector2f(MouseWindow))
    )
    {
        if ((M_TileSelector->IsActive() || M_CollisionAdder->IsActive()) && ShowSelector)
        {
            // Parsing the data and pass it to the variable
            if (StateName == DEFAULT)
            {
                // Getting data from tile selector
                std::map<String, String> TileData = M_TileSelector->GetData();
            ...
            else if (StateName == COLLIDER)
            {
                // Getting the Vertices from Collision Adder
                std::map<String, String> VerticesData = M_CollisionAdder->GetData();

                std::stringstream VerticesStream(VerticesData["Vertices"]);
                std::vector<sf::Vector2f> Vertices;
                sf::Vector2f Vertex;

                while (VerticesStream >> Vertex.x >> Vertex.y)
                ...
                !M_CollisionAdder->GetBounds().contains(sf::Vector2f(MouseWindow))
        )
            if (StateName == DEFAULT)
                Map->AddTile(MouseView, TextureFile, ShowGrid, Collision, Type);
            else if (StateName == COLLIDER)
                Map->AddCollision(MouseView);
    }
    // *** END *** //
...
void EditorState::UpdateSelectorInfo()
{
    ...
    String tile_type_str[2] = { "Normal", "Defered" };
    String collision_str[2] = { "OFF", "ON" };
    String state_name_str[2] = { "Default", "Collider" };

    IS << "Current State: " << state_name_str[StateName] << std::endl
       << "Position: " << MouseView.x << " x " << MouseView.y << std::endl
...
void EditorState::HandleModals()
{
    Modals["SAVE_MODAL"]->OnContinue([this]() {
        ...
        String MapName = Modals["SAVE_MODAL"]->GetInput();
    ...
    Modals["LOAD_MODAL"]->OnContinue([this]() {
        ...
        String MapName = Modals["LOAD_MODAL"]->GetInput();
...
void EditorState::OnInput(sf::Event* Event)
{
    ...
    else if (Event->type == sf::Event::MouseWheelScrolled && !M_TileSelector->IsActive() && !M_CollisionAdder->IsActive())
    {
        // *** Zoom in-out Camera *** //
...
void EditorState::OnDraw(sf::RenderWindow* Window)
{
    ...
    /* Map */
    Map->OnDraw(Window, nullptr, MouseGrid);
    Map->DrawDefered(Window, nullptr);
