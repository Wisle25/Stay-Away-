    ...
    HeartBeatPlayed(false)
{
    ...
    // Music
    Backsound.openFromFile("../Resources/sounds/Backsound.ogg");
    Backsound.setLoop(true);
    Backsound.setVolume(48.f);
    Backsound.play();

    HeartBeat.openFromFile("../Resources/sounds/Heartbeat.ogg");
    HeartBeat.setLoop(true);
    Backsound.setVolume(80.f);
}
...
void GameState::InitializeEntities()
{
    ...
    M_Player = new Player({
        ...
        // Start Position
        .Position = {
            Randomize<float>(64.f, Map->GetMaxSize().x * Ctx->GridSize),
            Randomize<float>(64.f, Map->GetMaxSize().y * Ctx->GridSize),
        },
...
void GameState::HandleInputPlayer(const float& DeltaTime)
{
    ...
    )
    {
        ...
        M_Player->GetAttribute()->Stamina -= 10.f * DeltaTime;

        // Reset the stamina to 0
        if (M_Player->GetAttribute()->Stamina < 0.f)
            M_Player->GetAttribute()->Stamina = 0.f;
    }
    else
    {
        ...
        if (M_Player->GetAttribute()->Stamina <= 100.f)
            M_Player->GetAttribute()->Stamina += 15.f * DeltaTime;

        // Reset the stamina to 100
        if (M_Player->GetAttribute()->Stamina > 100.f)
            M_Player->GetAttribute()->Stamina = 100.f;
    }
...
void GameState::HandleCombat(const float& DeltaTime)
{
    ...
    // Play the music for heartbeat once the player HP is under 30
    if (M_Player->GetAttribute()->HP <= 30.f && !HeartBeatPlayed)
    {
        HeartBeat.play();
        Backsound.setVolume(25.f);

        HeartBeatPlayed = true;
    }
    else if (HeartBeatPlayed)
    {
        HeartBeat.stop();
        Backsound.setVolume(48.f);
    }
...
void GameState::SpawnHatch()
{
    ...
    if (Objectives.size() <= 0 && M_Hatch == nullptr)
        M_Hatch = new Hatch(Cvt, {1.f, 1.f}, RandomizePosition(Map->GetMaxSize() * Ctx->GridSize));
...
{
    // If the player is dead, he'll get jumpscared :)
    ...
    // If the player escaped, will get "Escaped screen" instead
    if (Finished)
    {
        M_Hatch->OnUpdate(DeltaTime);
