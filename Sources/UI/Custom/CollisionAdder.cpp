...
const std::map<String, String> CollisionAdder::GetData() const
{
    return M_ScrollMode->GetData();
}

const std::vector<sf::Vector2f> CollisionAdder::FindVerticesByName(const String& Name)
{
...
const bool CollisionAdder::IsActive() const
{
    return Active;
}
...
void CollisionAdder::InitializeCollisions()
{
    ...
    while (InFile.read((char*)&NameSize, sizeof(unsh)))
    {
        ...
        String Information = InformationBuffer;
...
void CollisionAdder::OnInput(sf::Event* Event)
{
    ...
    // Removing Collider
    if (Event->type == sf::Event::KeyPressed)
        if (Event->key.control && Event->key.code == sf::Keyboard::Delete)
        {
            std::cout << "Test" << std::endl;

            delete Colliders[Colliders.size() - 1];
            Colliders.pop_back();
            CollidersNames.pop_back();
...
{
    // *** Updating is the Adder is active or not *** //
    if (Container.getGlobalBounds().contains(sf::Vector2f(MouseWindow)))
        Active = true;
    else
        Active = false;
    ...
    String str_state[3] = { "Normal", "Adding", "Finished" };
